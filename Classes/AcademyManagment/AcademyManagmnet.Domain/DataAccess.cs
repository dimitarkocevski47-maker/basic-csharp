...
namespace AcademyManagmnet.Domain;
...
public class DataAccess
{
    ...
    {
        Admin? adminFromDb = _db.Admins.FirstOrDefault(x => x.Username == username && x.Password == password);
        return adminFromDb;
    }

    public Trainer? GetTrainer(string username, string password)
    {
        Trainer? trainerFromDb = _db.Trainers.FirstOrDefault(x => x.Username == username && x.Password == password);
        return trainerFromDb;
    }

    public Student? GetStudent(string username, string password)
    {
        Student? studentFromDb = _db.Students.FirstOrDefault(x => x.Username == username && x.Password == password);
